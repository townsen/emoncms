    ...
    public function process_list() {
        ...
        $list = array(
           ...
           array(
              ...
              "unit"=>"",
              "group"=>_("Virtual"),
              "description"=>_("<p>Return the reciprocal of the specified feed. Returns NULL for zero values.</p>")
           ),
           array(
              "id_num"=>60,
              "name"=>_("Source Cost Feed"),
              "short"=>"scost",
              "argtype"=>ProcessArg::FEEDID,
              "function"=>"source_cost_feed",
              "datafields"=>1,
              "unit"=>"",
              "group"=>_("Virtual"),
    ...
    public function source_cost_feed($feedid, $time, $value, $options)
    {
        ...
        if (isset($options['start']) && isset($options['end'])) {
            ...
            if (!isset($this->data_cache[$feedid])) {
                ...
                $this->log->debug("source_cost_feed() loaded to cache");
            }
            // Return value
            if (isset($this->data_cache[$feedid][$options['index']])) {
