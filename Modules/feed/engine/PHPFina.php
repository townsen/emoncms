...
// #### \/ Below are required methods
    ...
    */
    public function scalerange($id,$start,$end,$scale){
        ...
        } elseif(preg_match("/^(1\/|-1\/|-)?([0-9]+((\.|,)[0-9]+)?)$/",$scale,$a)){
        ...
        } elseif(preg_match("/^abs\(x\)$/i",$scale)){
...
// #### \/ Bellow are engine private methods
    ...
    public function get_sha256sum($id, $npoints = 0)
    {
        ...
        $bytes_to_read = $npoints * 4;
        $dat_file = $this->dir . $id . ".dat";
        ...
        return array(
            'success' => true,
            'npoints' => $npoints,
            'size' => number_format(($npoints * 4)/(1024*1024),1)." MB",
            'sha256sum' => $parts[0]
        );
    }
}
